...
namespace :resque do
  ...
  task :restart_workers, [:num_workers, :queue] => :environment do |t, args|
    kill_unregistered_workers
    stop_workers
    count = args[:num_workers].to_i
    queue = args[:queue] || "*"
    count = count > 0 ? count : 1
    run_worker(queue, count)
  end
  ...
  desc "Start workers"
  task :start_workers, [:num_workers, :queue] => :environment do |t, args|
    count = args[:num_workers].to_i
    queue = args[:queue] || "*"
    count = count > 0 ? count : 1
    run_worker(queue, count)
  ...
  def stop_workers
    ...
    until workers.empty? do
      workers.each_index do |i|
        ...
        if w.idle?
          workers.delete_at(i)
          resque_log "Stopping worker #{pid}..."
          w.unregister_worker
          system("kill #{pid}")
        else
          resque_log "Waiting for worker #{pid} to go idle..."
          sleep 1
          next
        end
      end
  ...
  def run_worker(queue, count = 1)
    resque_log "Starting #{count} worker(s) with QUEUE: #{queue}"
    ...
    count.times do
      ...
      Process.detach(pid)
      resque_log "Started worker with PID #{ pid }"
    end
